...
function WorkDetailsSectionWrapper() {
  ...
  return (
    <WorkDetailsSection register={register} errors={errors} control={control} />
  );
}
...
describe('WorkDetailsSection - Rendering', () => {
  ...
  it('should display max words label', () => {
    ...
    expect(screen.getByTestId('max-words-label')).toHaveTextContent(
      'Max 500 mots'
    );
  });
...
describe('PdfUploadSection - Rendering', () => {
  ...
  it('should display drop zone text', () => {
    ...
    expect(screen.getByTestId('drop-text-main')).toHaveTextContent(
      'Glissez votre fichier PDF ici'
    );
    expect(screen.getByTestId('drop-text-secondary')).toHaveTextContent(
      'ou cliquez pour parcourir'
    );
    expect(screen.getByTestId('file-constraints')).toHaveTextContent(
      'PDF uniquement, max 10 Mo'
    );
  });

  it('should display file name when file is selected', () => {
    const mockFile = new File(['test content'], 'test-manuscript.pdf', {
      type: 'application/pdf',
    });

    render(
      <PdfUploadSection file={mockFile} onFileChange={mockOnFileChange} />
    );
...
describe('Synopsis Word Count - Requirement 3.7', () => {
  it('should update word count as user types', async () => {
    ...
    fireEvent.change(synopsisTextarea, {
  ...
  it('should display warning when synopsis exceeds 500 words', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('synopsis-warning')).toHaveTextContent(
...
describe('PDF File Validation - Requirements 4.8, 4.9', () => {
  ...
  it('should return null for valid PDF file', () => {
    const validPdf = new File(['test content'], 'valid.pdf', {
      type: 'application/pdf',
    });
...
describe('PdfUploadSection - Drag & Drop Behavior', () => {
  ...
  it('should handle drop event with valid PDF file', () => {
    ...
    const dropZone = screen.getByTestId('drop-zone');
    const validPdf = new File(['test content'], 'test.pdf', {
      type: 'application/pdf',
    });
