 ...
 */
const blogArticleArbitrary = (index: number) =>
  fc.record({
...
describe('Property 1: Article Card Content Completeness', () => {
  it('should render all required content elements for any valid article', () => {
    fc.assert(
      fc.property(threeArticlesArbitrary, articles => {
        ...
        try {
          ...
          for (const article of articles) {
            ...
            const allTitles = Array.from(container.querySelectorAll('h3')).map(
              el => el.textContent
            );
            expect(allTitles).toContain(article.title);

            // 3.5: Excerpt should be displayed
            const allExcerpts = Array.from(
              container.querySelectorAll('.text-light-dimmed.leading-relaxed')
            ).map(el => el.textContent);
            expect(allExcerpts).toContain(article.excerpt);

            // 3.2: Category badge should be displayed
            const allCategories = Array.from(
              container.querySelectorAll('.bg-gold.text-dark')
            ).map(el => el.textContent);
            ...
            const coverImages = container.querySelectorAll(
            ...
            const allAuthors = Array.from(
              container.querySelectorAll('.text-light.font-medium')
            ).map(el => el.textContent);
    ...
    fc.assert(
      fc.property(threeArticlesArbitrary, articles => {
        ...
        try {
          ...
          const categoryBadges = container.querySelectorAll(
            '.bg-gold.text-dark.uppercase'
          );
...
describe('Property 2: Author Info Completeness', () => {
        ...
        try {
          for (const article of articles) {
            ...
            const allDates = Array.from(
              container.querySelectorAll('.text-light-dimmed')
            ).map(el => el.textContent);
          ...
          const separatorTexts = Array.from(separators).filter(
            el => el.textContent === '•'
          );
  ...
  it('should display avatar image when provided', () => {
    ...
    fc.assert(
      fc.property(articlesWithAvatarsArbitrary, articles => {
        const { container, unmount } = render(
          <BlogSection articles={articles} />
        );
  ...
  it('should display initials fallback when no avatar is provided', () => {
    ...
    fc.assert(
      fc.property(articlesWithoutAvatarsArbitrary, articles => {
        const { container, unmount } = render(
          <BlogSection articles={articles} />
        );
  ...
  it('should display "EG" initials for "ÉDITIONS GERMINALE" author', () => {
    ...
    fc.assert(
      fc.property(editionsGerminaleArbitrary, articles => {
        const { container, unmount } = render(
          <BlogSection articles={articles} />
        );
    ...
    fc.assert(
      fc.property(threeArticlesArbitrary, articles => {
        ...
        try {
          ...
          const altTexts = Array.from(coverImages).map(img =>
            img.getAttribute('alt')
          );

          // Each article title should appear in one of the alt texts
          for (const article of articles) {
            const hasMatchingAlt = altTexts.some(alt =>
              alt?.includes(article.title)
            );
