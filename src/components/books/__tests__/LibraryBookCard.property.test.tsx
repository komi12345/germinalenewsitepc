...
const bookExtendedArbitrary = fc.record({
  id: fc.uuid(),
  title: fc
    .stringMatching(/^[A-Za-z0-9àâäéèêëïîôùûüç\s\-']{3,50}$/)
    .filter(s => s.trim().length >= 3),
  slug: fc
    .stringMatching(/^[a-z0-9]+(-[a-z0-9]+)*$/)
    .filter(s => s.length >= 3 && s.length <= 50),
  author: fc
    .stringMatching(/^[A-Za-z\s\-'àâäéèêëïîôùûüç]{3,50}$/)
    .filter(s => s.trim().length >= 3),
  ...
  it('should display "Coup de cœur" badge if and only if isFeatured is true', () => {
    fc.assert(
      fc.property(bookExtendedArbitrary, (book: BookExtended) => {
        ...
        const featuredBadge = container.querySelector(
    ...
    fc.assert(
      fc.property(bookExtendedArbitrary, (book: BookExtended) => {
        ...
        const expectedBadgeCount =
          (book.isNew ? 1 : 0) + (book.isFeatured ? 1 : 0);
