...
describe('Property 2: Password Validation Rules', () => {
        ...
        fc.property(
          ...
          (shortPassword) => {
            ...
            if (!result.success) {
              ...
              return passwordError?.message === AUTH_ERROR_MESSAGES.PASSWORD_TOO_SHORT;
            }
            return false;
          }
        ),
      ...
      fc.assert(
        fc.property(
          ...
          (shortPassword) => {
            ...
            if (!result.success) {
              const passwordError = result.error.issues.find(
                issue => issue.path && issue.path.includes('password') && !issue.path.includes('confirmPassword')
              );
              return passwordError?.message === AUTH_ERROR_MESSAGES.PASSWORD_TOO_SHORT;
            }
            return false;
  ...
  describe('Valid password length validation', () => {
    ...
    it('should accept passwords with exactly 8 characters', () => {
      ...
      const exactLengthPasswordArbitrary = fc.string({ minLength: 8, maxLength: 8 });

      fc.assert(
        fc.property(
          exactLengthPasswordArbitrary,
          (password) => {
        ...
        fc.property(
          ...
          (emptyPassword) => {
            ...
            if (!result.success) {
              ...
              return passwordError?.message === AUTH_ERROR_MESSAGES.REQUIRED_FIELD;
            }
            return false;
          }
        ),
...
describe('Property 3: Password Match Verification', () => {
  ...
  describe('Non-matching passwords validation', () => {
        ...
        fc.property(
          ...
          ([password, confirmPassword]) => {
            ...
            if (!result.success) {
              ...
              return matchError?.message === AUTH_ERROR_MESSAGES.PASSWORDS_DONT_MATCH;
            }
            return false;
          }
        ),
    ...
    it('should reject when passwords differ by case', () => {
      ...
      const caseDifferentPasswordsArbitrary = fc.string({ minLength: 8, maxLength: 20 })
    ...
    it('should reject when passwords differ by whitespace', () => {
      ...
      const whitespaceDifferentArbitrary = fc.string({ minLength: 8, maxLength: 20 })
          ...
          (password) => {
            ...
            if (!result.success) {
              ...
              const hasMatchError = result.error.issues.some(
                issue => issue.message === AUTH_ERROR_MESSAGES.PASSWORDS_DONT_MATCH
              );
...
describe('Property 4: Required Field Validation', () => {
    ...
    it('should reject empty confirmPassword field', () => {
      fc.assert(
        fc.property(
          fc.constant(''),
          (emptyValue) => {
            const data = { ...createValidRegisterData(), confirmPassword: emptyValue };
            const result = registerSchema.safeParse(data);
            if (!result.success) {
              const error = result.error.issues.find(
                issue => issue.path && issue.path.includes('confirmPassword')
  ...
  describe('All fields empty validation', () => {
    ...
    it('should reject register form with all empty fields', () => {
      fc.assert(
        fc.property(
          fc.constant({ fullName: '', email: '', password: '', confirmPassword: '' }),
          (emptyData) => {
